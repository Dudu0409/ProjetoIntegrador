...
import MidiaUsuarioList from "./MidiaUsuarioList";
import MidiaUsuarioForm from "./MidiaUsuarioForm";
import MidiaUsuarioSrv from "./MidiaUsuarioSrv";
...
function MidiaUsuarioCon() {
  ...
  const onClickAtualizar = () => {
    MidiaUsuarioSrv.listar()
      .then((response) => {
        setMidiasUsuarios(response.data);
        toastRef.current.show({
          severity: "success",
          summary: "Mídias de usuários atualizadas!",
          life: 3000,
        });
      })
      .catch((e) => {
  ...
  const editar = (id) => {
    setMidiaUsuario(midiasUsuarios.filter((midiaUsuario) => midiaUsuario._id === id)[0]);
    setEditando(true);
  };
